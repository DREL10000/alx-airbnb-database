...
-- Use these queries with EXPLAIN or EXPLAIN ANALYZE to measure performance
-- before and after creating indexes

-- BEFORE CREATING INDEXES: Run these commands
...
-- AFTER CREATING INDEXES: Run the same EXPLAIN commands to compare execution plans
-- Look for:
-- - Index usage (type should show 'ref' or 'range' instead of 'ALL')
-- - Reduced rows examined
-- - Faster execution times with EXPLAIN ANALYZE
...
-- SELECT * FROM sys.schema_index_statistics; (MySQL 8.0+)

-- Consider periodic index maintenance:
